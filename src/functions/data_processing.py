...
class DataAnalytics:
    def __init__(self, api_parser, data_parser, helper_fns):
        ...
        self.optimized_personal_team_df = self.optimize_fpl_team() #df_fpl
        self.prospects_df = self.compile_all_prospects() #df_fpl_potentials
        self.primes_df = self.condense_prospects() #df_prime_potentials
...
#============================================  FPL EVALUATIONS  ============================================
    ...
    def grab_top_players_by_mean(self, list_of_ids: list = None, is_for_personal_fpl_team = False) -> pd.DataFrame:
        ...
        if self.api_parser.latest_gw == 1:
            LOOK_BACK_LIST = [1]
        elif self.api_parser.latest_gw  == 2:
            LOOK_BACK_LIST = [2]
        elif self.api_parser.latest_gw  < 6:
            LOOK_BACK_LIST = [3,2]
        else:
            LOOK_BACK_LIST = [6,3,2]
        returns_nontop6={}
        for idx in list_of_ids:
            recent_returns_bank = self.data_parser.total_summary.loc[self.data_parser.total_summary['id_player'] == idx]['returnhist'].iloc[0]
        ...
        for num, ids in enumerate(tqdm_notebook(df['id'], desc = desc)):
            recent_returns = self.data_parser.total_summary.loc[self.data_parser.total_summary['id_player'] == int(ids)]['returnhist'].iloc[0][-6:]
            count = recent_returns.count(1) + recent_returns.count(2)
    ...
    def compile_fpl_team(self):
        r = self.api_parser.personal_fpl_raw_data
        fpl_team_ids = set([x['element'] for x in r['picks']])
        df = self.data_parser.total_summary[self.data_parser.total_summary['id_player'].isin(fpl_team_ids)].reset_index(drop=True)
...
#============================================  SUMMARIZING  ============================================
    ...
    def form_dataframes(self):
        ...
        self.MT_df = self.condense_and_merge_dataframes(self.optimized_personal_team_df.iloc[:, 0:5])
        ...
        def process_replacements():
            ...
            for param,param_thresh in [('ict_index',7),('returnhist',1),('bps',25),('expected_goal_involvements',0.75),('history',6)]:
                ...
                for look_back in [1,2,3,4,5,6]:
                    df = self.data_parser.total_summary.copy()
                    FPL_15_players = self.optimized_personal_team_df.id.tolist()
                    ...
                    if key == 'DEF':
                        ...
                        if np.mean(xGI[-6:]) > 0.4 or passes_xGI_threshold(xGI):
                            self.replacement_players.append({'id': num,
                                                            ...
                                                            'minutes': mins[-6:],
                                                            'cost': cost/10})
            players = [x for x in self.players if x['id'] in self.optimized_personal_team_df['id'].to_list()]    
